...
{{-- ================= HERO SECTION (STYLE MATCHING) ================= --}}
<div class="relative py-24 md:py-32 lg:py-40 overflow-hidden">
    
    {{-- 1. Background Image --}}
    <div class="absolute inset-0">
        {{-- Gambar default untuk halaman indeks berita --}}
        <img 
            src="{{ asset('storage/images/default-news-cover.jpg') }}" 
            ...
            <div class="lg:col-span-8 space-y-10">
               @forelse($posts as $post)
                <article class="group relative bg-white rounded-[2rem] overflow-hidden border border-slate-100 shadow-lg hover:shadow-xl transition-all duration-300 flex flex-col md:flex-row mb-8">
                    ...
                    {{-- 1. Thumbnail Area (DIPERBAIKI) --}}
                    {{-- Kita kunci lebarnya di desktop (w-72) dan pakai aspect ratio biar ga melar --}}
                    ...
                    <div class="p-6 md:p-8 flex flex-col justify-between flex-1">
                        ...
                        <div class="flex items-center text-sky-600 font-bold text-xs uppercase tracking-wider mt-auto">
                            <a href="/posts/{{ $post->slug }}" class="inline-flex items-center gap-2 group">
                    ...
                    <div class="space-y-3">
                        @foreach($categories as $cat)
                            <a href="/posts?category={{ $cat->slug }}" 
                               class="group flex items-center justify-between p-4 rounded-2xl border border-slate-50 transition-all duration-300 {{ request('category') == $cat->slug ? 'bg-sky-600 text-white shadow-lg shadow-sky-500/30' : 'bg-slate-50 text-slate-600 hover:bg-sky-50 hover:border-sky-100' }}">
                                <span class="font-bold text-sm">{{ $cat->name }}</span>
                                
                                {{-- Counter Badge --}}
                                <span class="px-2.5 py-1 rounded-lg text-[10px] font-bold transition-colors {{ request('category') == $cat->slug ? 'bg-white/20 text-white' : 'bg-white text-slate-400 group-hover:text-sky-600' }}">
                                    {{ $cat->posts_count }}
                                </span>
                            </a>
                        @endforeach
                ...
                <div class="bg-white p-8 rounded-[2.5rem] shadow-xl shadow-sky-900/5 border border-slate-100 reveal-on-scroll" data-aos-delay="200">
                    ...
                    <div class="space-y-6">
                        @foreach($latest_posts as $latest)
                            <a href="/posts/{{ $latest->slug }}" class="flex gap-4 group items-start">
